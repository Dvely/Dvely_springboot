...
## 4.27 프리뷰 게이트웨이 프록시 타깃을 컨테이너 IP 로 (Issue #358)

**되는 것**

- 프리뷰 컨테이너가 **호스트 포트를 발행하지 않는다.** 게이트웨이는 `preview_sessions.container_ip` 로 `http://{ip}:3000` 에 프록시한다
- 포트 할당·충돌·재할당 문제가 사라졌다 — 포트가 고정 3000 이고 주소로 가른다. #278(host_port 재할당)이 막던 자리가 원인째 없어졌다
- 배포 시점에 열려 있던 세션도 끊기지 않는다. `container_ip` 가 비어 있으면 게이트웨이가 첫 요청에서 컨테이너를 조회해 채운다(지연 해석)
- 컨테이너 재시작 후 주소 갱신은 `rebindContainerIp` 로 이어진다(#71 과 같은 경로, 원인만 포트 → IP 로 바뀜)

**노출면**

호스트에 열리는 포트가 하나도 없다. 브리지 대역은 호스트에서만 라우팅되고 컨테이너 사이는 `enable_icc=false` 가 막는다. 루프백 발행(BI-081/G1)보다 강한 상태다.

**왜 했나**

#332 4단계(egress 허용목록)가 이 구조에 막혀 있었다. egress 를 실제로 차단하려면 `--internal` 네트워크가 필요한데 **거기서는 포트 발행이 동작하지 않는다**(실측: 매핑이 생기지 않음). 반면 호스트는 컨테이너 IP 로 닿는다(200).

egress 와 한 PR 로 묶지 않은 이유는 게이트웨이가 #274(회수/self-heal)·#278(포트 재할당)·#281(레디니스 게이트)이 쌓인 자리여서다 — 함께 바꾸면 무엇이 깨졌는지 가려낼 수 없다.

**아직 안 되는 것**

- egress 허용목록 자체 (#332 4단계). 이 단위가 그 선행 조건을 푼 것이고, 네트워크를 `--internal` 로 바꾸는 것은 다음이다



## 4.28 프리뷰 egress 허용목록 (Issue #332 4단계) — 기본 비활성

**되는 것 (켰을 때)**

- 프리뷰마다 전용 `--internal` 네트워크. 프록시 하나가 그 전부와 `bridge` 에 붙어 허용목록만 통과시킨다
- 격리가 `enable_icc=false` 보다 **강하다** — 서로 다른 네트워크라 라우팅 경로 자체가 없다(TCP·ICMP 모두 차단 확인)
- 게이트웨이는 무영향. `--internal` 네트워크의 컨테이너에도 호스트에서 IP 로 닿는다(#358 이 선행 조건이었다)
- 프록시 경유로 동작 확인: `apk`(root) · `curl` · `git clone` · `npm install`

**왜 공유 네트워크가 아닌가 (실측)**

| | `icc=false` | `icc=true` |
|---|---|---|
| 허용 호스트(프록시 경유) | **000 막힘** | 200 |
| 컨테이너 간 | 차단 | **도달됨** |

`icc=false` 는 앱→프록시까지 막는다. 공유 네트워크로는 egress 필터링과 멀티테넌시 경계 중 하나만 된다.

**함께 바뀐 것 — 씨딩이 `wget` → `curl`**

busybox `wget` 은 프록시 CONNECT 터널링을 하지 않는다. egress 를 켜면 요청을 보내지도 않고 소켓을 닫아 tinyproxy 로그에만 흔적이 남는다. **모드에 따라 도구를 가르지 않고** 양쪽 다 `curl` 을 쓴다 — 조용히 갈라지는 경로를 만들지 않는다. 이 변경은 플래그와 무관하게 적용된다.
