...
프리뷰 컨테이너가 **root(uid 0)로 돌고**, egress 가 제한되지 않는다. 비-root 로 돌리는 방법은 실측으로 확인했지만(root 로 워크스페이스를 만들어 넘기고 이후 exec 를 `--user node` 로), `apk add`·nginx 처럼 root 가 필요한 명령과 섞여 있어 쓰기 주체를 한 번에 옮겨야 하는 조율 작업이다.
