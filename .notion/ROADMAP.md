...
- 2026-09-26: **템플릿 카탈로그가 FE 전체의 단일 장애점이 됐고, 그 두 구멍을 막았다 — develop (#395 PR #396·#397, V65). 릴리스 남음**.
  발단은 `Dvely_FE#117`(`41a20f3`)이 프로덕션에 들어가면서 FE 의 더미 템플릿 목록이 사라진 것이다. 이제 **랜딩·갤러리·홈 탐색·생성 미리보기·프롬프트 얹기 다섯 화면이 전부 `GET /api/v1/templates` 하나만 본다.** 예전에는 실패해도 갤러리만 깨졌다.

  **① 재시작마다 cold start 였다.** 캐시가 `private volatile Snapshot` 이라 메모리에만 있었고 **배포가 곧 그 순간**이다. `request-timeout: 5s` 안에 GH Pages 가 답하지 못하면 503 이다. 성공 갱신을 `template_catalog_snapshot`(단일 행, V65)에 남기고 cold start 에서 네트워크가 실패했을 때만 복원한다. 복원 조건 셋 — 출처 URL 일치 · 나이 ≤ `snapshot-max-age`(기본 24h) · 스킴 재검사. 하나라도 어긋나면 예전처럼 503 이다.
  **jar 에 폴백 카탈로그를 심는 안은 일부러 뺐다.** 낡은 목록을 보여주면 없어진 템플릿을 고르게 되고, `TemplateCatalogGuard` 는 같은 스냅샷을 읽으니 통과시켜 씨딩에서 깨진다 — "고를 때는 200, 만들 때는 실패"는 그 guard 가 존재하는 이유 그 자체다.

  **② 카탈로그의 URL 을 아무도 검사하지 않았다.** 외부 JSON 을 바로 역직렬화해 통과시키고 검사는 "비어 있나" 하나였다. FE 가 `demoUrl` 을 iframe `src` 에, `thumbnailUrl` 을 `img src` 에 넣는다. 수신 지점에서 `http(s)` 를 확인하고, **나쁜 템플릿 하나를 빼지 않고 문서를 거부**해 stale-while-error 에 맡긴다 — 조용히 빼면 카탈로그를 고친 사람은 자기가 무엇을 잘못했는지 모른 채 그 템플릿이 사라진 것만 본다.
  `sourceUrl` 은 **이미 안전했다** — 셸 `curl` 에 들어가는 자리라 `^https://[A-Za-z0-9._~:/-]+\.tar\.gz$` 가 **소비 지점**에 걸려 있다. 가장 위험한 자리가 가장 잘 막혀 있고 브라우저로 나가는 둘이 비어 있었다. 싱크의 좁은 검사를 수신 지점으로 옮기거나 느슨하게 복제하지 않았다.

  **`@Transactional` 을 붙여 한 번 틀렸다.** `save` 에 붙였더니 커밋을 프록시가 메서드 **밖에서** 하므로 플러시 실패가 `try/catch` 를 지나쳐 올라가고, cold start 면 **네트워크가 성공했는데도 503** 이 된다 — 보조 장치가 본 기능을 죽인다. 게다가 **단위 테스트에는 프록시가 없어 붙인 채로도 통과한다**("저장 실패가 서빙을 막지 않는다" 테스트가 그렇게 거짓 통과했다). 트랜잭션을 빼고 `repository.save` 하나(merge)로 바꾸고, **애노테이션 부재를 직접 단정하는 테스트**로 고정했다.

  **검증**: 전체 1762건 통과. **역검증 6건** — 스킴 검사 제거 3건 / 복원 호출 제거 2건 / 저장 호출 제거 5건 / 나이 상한 제거 1건 / 출처 검사 제거 1건 / `@Transactional` 재부착 1건. 나이·출처는 store 안에 있어 클라이언트 쪽 되돌리기로는 **엉뚱한 이유(NPE)로** 실패했으므로 그 둘만 따로 떼어 단정 실패로 잡히는 것까지 봤다.
  복원 왕복은 목이 아니라 **진짜 Jackson + 실제 MySQL** 을 태운다(PR #397). 복원은 평소에 돌지 않고 **정작 필요한 순간에만** 도는 코드라 조용히 깨져 있으면 아무도 모른다. 민감도도 확인했다 — `fetched_at` 을 `datetime(6)` → `datetime` 으로 깎으면 실패한다(`ddl-auto: validate` 는 이 차이를 못 잡는다).
  그 민감도 확인에서 **거짓 통과를 한 번 봤다** — "0건 실패"가 나왔는데 실은 gradle 이 실패하고 내가 직전 실행의 XML 을 읽고 있었다. **실패 수만 세면 "안 돌았다"와 "전부 통과"가 같은 값으로 보인다.**
  **dev 실측**: `flyway V65 success` · 스냅샷 행 `payload=17272B · 17종` · 출처 URL 일치 · 카탈로그 갱신 로그 정상 · ERROR 0.

  **`#391` 은 운영 배포 완료**(PR #394, `01a5109`). 승인 메시지에서 죽는 preview 링크를 뺐다 — 배포된 jar 에서 두 게이트 클래스의 URL 리터럴 0개를 직접 확인했다. 뿌리인 이름 충돌은 **`#392`** 로 떼고 FE 와 3단계 순서를 합의했다(BE 새 이름 추가 → FE 이전 → BE 옛 이름 제거). **`previewUrl` 은 계약이 셋**이고 셋의 실패 코드가 다르다 — access 만 열리고, 세션 응답은 쿠키가 있을 때만(**401**), 태스크 응답은 낡은 스냅샷(**404**)이다. ⚠️ `require-access-cookie` 가 **dev `false` / 운영 `true`** 라 **dev 검증은 반드시 통과한다.**

  **`#318` 은 이제 닫을 수 있다** — BE 는 끝났고 FE 갤러리도 프로덕션에 나갔다(`#115` `#117`).

  **Cloudflare 는 사흘 연속이다.** 릴리스 직후 헬스가 522(46초)로 떨어져 같은 시점에 두 경로를 나란히 쟀다 — **오리진 15/15 ~10ms · 엣지 9/15(colo=SJC)**. 배포 탓이 아니고 코드로 못 고친다. 순서는 ①Tiered Cache ②AWS 보안그룹 ③**오리진 서울 이전**이며 전부 사용자 손이 필요하다.
...
- 2026-09-22: **운영 느림·끊김의 원인을 구간별 실측으로 규명하고 셋 다 고쳤다 — main 배포 완료(#370 #372, PR #374)**.
  ...
  **① nginx 설정 드리프트.** 운영 `/api/` 에 `proxy_read_timeout` 이 없어 기본 60초였다. **dev 에는 3600초로 주석까지 달려 있었다** — 서버 설정이 레포 밖에 있어 한쪽만 고쳐진 채 아무도 못 봤다. 정적 자산에는 `Cache-Control` 이 아예 없어 Cloudflare 기본 4시간만 잡혔고, 297KB 번들을 4시간마다 싱가포르에서 다시 끌어왔다. `/assets/` 에 `immutable`, `/index.html` 에 `no-cache`, `/api/` 에 dev 와 같은 SSE 설정을 넣었다(백업 `dvely.bak-20260922-113300`).

  **② 그것만으로는 절반이었다(#370).** `AgentEventStreamService` 가 이벤트가 있을 때만 `send()` 해서, `afterEventId` 가 최신 커서인 재연결은 **응답 헤더조차 안 나갔다**. 운영 로그가 `afterEventId` 로 정확히 갈린다 — `=0` 은 replay 덕에 200, `=190` 은 `while reading response header from upstream` 으로 504. **첫 연결은 되고 재연결이 죽는 구조**였고, FE 는 연속 3회 실패로 폴링에 내려앉은 뒤 그 태스크 동안 복구하지 않아 사용자가 25분간 빈 폴링을 봤다(로그 비율 폴링 122 : 스트림 12). 열자마자 주석 프레임 1회 + 유휴 15초 반복으로 고쳤다.

  **③ 곁가지로 나온 진짜 버그(#372).** `NoResourceFoundException` 핸들러가 없어 인증된 호출의 오타 경로가 404 가 아니라 **500** 이었다. FE 세션이 그 500 을 "서버가 터졌다"로 읽고 시간을 버린 것이 발견 계기다.

  **검증**: 전체 1719건 통과. 두 변경 모두 **수정을 되돌려 테스트가 실제로 실패하는지 확인**했다(하트비트 3건, 404 는 `expected:<404> but was:<500>`). dev 에서 사고 조건을 직접 만들어(`afterEventId` 를 마지막 이벤트 이후로) `+00s~+61s :hb`, 75초 유지, nginx 200, timeout 0건 — **수정 전 그 자리는 0바이트였다.**

  **FE 협업**: `Dvely_FE` 세션과 교차 검증했다. FE 는 `EventSource` 가 아니라 `fetch`+`getReader()` 를 쓰므로 `Last-Event-ID` 지원은 불필요로 정리했고, 주석 프레임은 `spring-webmvc` jar 를 직접 열어 `:hb\n\n` 임을 확인해 FE 변경 0건을 확정했다. FE 쪽에서는 별건으로 `enabled` 토글마다 커서가 0 으로 되돌아가던 버그를 고쳤다(FE #106) — dev 로그에서 재연결 커서가 `941` 로 이어지는 것을 확인했다.

  **남은 것**: Cloudflare Tiered Cache 는 대시보드 작업이라 미적용. 그리고 **엣지 선택이 지연의 지배 변수**다 — 오전엔 HKG(0.120s)/NRT(0.223s) 였는데 21시경 같은 애니캐스트 IP 가 **SEA(시애틀)** 로 바뀌어 0.32s 가 기본이 됐다(cloudflare.com 은 같은 회선에서 ICN). 오리진이 싱가포르인 한 이 변동은 남는다.

- 2026-09-21: **문서 정합 정리 — 빠져 있던 절 3개와 번호 충돌 해소**.

  `state.md` 에 **§4.22 가 둘**이었다(PAT #304 와 템플릿 #318). "§4.22 를 보라" 가 두 곳을 가리키는 상태라 템플릿을 **§4.29** 로 옮겼다 — `api.md` 에서 같은 일이 한 번 있었고(Template 과 ApiToken 이 §17 충돌) 그때 밀어서 해결한 것과 같은 처리다.

  빠져 있던 절을 채웠다: **§4.30 프리뷰 컨테이너 node 실행**(#332 1~3단계 — 그때 `connection.md`·ROADMAP 만 갱신하고 `state.md` 절을 안 만들었다), **§4.31 AI 모델 설정 검사기**(#362), **§4.32 AI 실행을 사용자 키로만**(#364/#365).

  **#365 는 문서를 하나도 갱신하지 않았다.** 코드 실측으로 정리해 넣었다 — `aiProvider` 가 `@NotNull` 이 됐고(깨뜨리는 변경), `application-{dev,prod}.yml` 에서 벤더 `api-key` 가 사라졌으며, `src/main` 의 `getApiKey()` 호출은 전부 `aiaccount` 안이라 배포 설정으로 떨어지는 갈래가 없다. `api.md` 의 메시지 생성 계약과 `connection.md` 의 모듈 표·키 경로에 반영했다.

  **⚠️ 운영에 죽을 예정인 키가 남아 있다.** 9/18 릴리스 때 넣은 `QEPLOY_AI_GLM_API_KEY`(와 기존 ANTHROPIC·OPENAI 키)는 **지금 운영(main)에서는 여전히 쓰인다** — #365 가 `develop` 에만 있기 때문이다. 다음 릴리스가 나가면 읽히지 않으므로, **그때 제거**해야 한다. 그 전에 지우면 운영이 깨진다. 모델·thinking 설정은 모델 카탈로그라 계속 쓰인다.

  **다음 릴리스 전 확인할 것**: `aiProvider` 필수화는 깨뜨리는 변경이다 — FE 가 이미 보내고 있는지(`Dvely_FE`), 키 미등록 사용자를 등록 화면으로 보내는 경로가 있는지 확인해야 운영에서 "메시지가 안 보내진다" 로 나타나지 않는다.
- 2026-09-21: **AI 실행을 사용자 본인 키로만 돌린다 — 서버 키 경로 제거**(Issue #364 / **PR #365 머지**, develop `4a31e78`). 마이그레이션 없음.

  **문제**: `qeploy.com` 공개 운영 중에 **로그인한 사용자의 AI 요청이 운영자 키로 실행되고 운영자 계정에 과금**됐다. 상한도 사용자별 할당도 없었다. FE(`Dvely_FE` #99)가 화면에서 서버 키 제공자를 걷어냈지만 서버는 여전히 받았다 — 브라우저 JWT 만 있으면 `curl` 로 부를 수 있었다.

  **고친 것**: 키를 구하는 경로를 `UserAiKeyResolver` 하나로 모았다. `src/main` 에서 `getApiKey()` 를 부르는 곳은 이제 거기뿐이고 배포 설정으로 떨어지는 갈래가 없다 — 이것이 "서버 키 경로가 없다"의 구조적 근거다. 라우터가 **호출자 본인 키를 바인딩한 포트**를 돌려주므로, 키는 포트를 만들 때 풀리고 미등록 사용자의 요청은 LLM 왕복을 시작조차 못 한다.

  **함께 고친 회귀 — 착수 시점에 운영은 이미 깨져 있었다**: FE #99 가 고를 수 있는 제공자를 코딩 에이전트로 좁혔는데 `LlmRouter` 가 `CLAUDE_CODE`·`CODEX` 를 예외로 떨어뜨리고 있었다. **사용자가 무엇을 입력하든 계획(DECISION) 단계에서 즉시 실패**하는 상태였다. 코딩 에이전트에 chat-completions 엔드포인트가 없다는 것이 이유였는데, 정작 그 CLI 가 읽는 키가 사용자가 이미 등록해 둔 벤더 키였다 — 이제 계획·채팅은 그 키로 chat-completions 를 타고 코드 생성만 CLI 가 맡는다.

  **깨뜨리는 변경**: `POST /conversations/{id}/messages` 의 `aiProvider` 필수화(생략 시 400) · 키 미등록 시 **모든** AI 경로가 `AI_CREDENTIAL_NOT_REGISTERED`(예전엔 코딩 에이전트만) · `GET /agent/ai-providers` 가 사용자 등록 키 기준(미등록이면 빈 배열) · 배포 실패 요약은 룰 기반 전용(`analysisSource` 항상 `RULE_BASED`).

  **검증**: 전체 **1,713 통과(실패 0)**. 가드 신설 — `LlmRouterByokTest`(코딩 에이전트가 계획·채팅을 탄다 · 사용자별 키가 섞이지 않는다) · `UserAiKeyResolverTest` · `LlmApiKeyIsolationTest` 확장(8명 × 25회 동시 호출로 본문의 사용자 표식과 헤더 키를 대조하고, `LlmRouter` 에 "첫 키를 캐시" 버그를 심어 실제로 실패하는 것까지 확인).

  **⚠️ 남은 운영 조치 — 순서를 지켜야 한다**: 이 변경은 `develop` 에만 있고 **운영(main)은 아직 서버 키로 돈다.** 그래서 ① **다음 릴리스(develop → main)가 나간 뒤에** 운영 pm2 에서 `QEPLOY_AI_*_API_KEY` 를 제거하고 ② 그다음 벤더 콘솔에서 그 키들을 폐기·재발급한다(공개 운영 중 노출 이력). **순서를 바꿔 릴리스 전에 지우거나 폐기하면 운영이 깨진다** — 위 항목의 경고와 같은 내용이다. ③ 기존 사용자에게 본인 키 등록 안내.

  **FE 후속**: FE 는 제공자 목록을 `models: []` 로 걸러 코딩 에이전트만 보여준다. **GLM 키만 등록한 사용자는 화면에서 고를 것이 없다**(GLM 은 코딩 에이전트 모드가 없다). API·MCP 로는 정상 동작하므로, 목록 필터를 "본인 키가 등록된 제공자"로 넓히는 것이 `Dvely_FE` 후속이다.

- 2026-09-18: **운영 서버 재개 — develop → main 릴리스 완료(PR #363)**. 운영은 9/3 빌드·DB V39 로 멈춰 있었고, 174 커밋과 **마이그레이션 25개(V40→V64)** 를 한 번에 반영했다.

  **결과**: 25건 전부 적용(실패 0, 3.5초), 데이터 보존(users 3 / projects 13), 기동 33초, ERROR 0, 프리뷰 기준 오리진 `https://qeploy.com` 정상. 외부에서 로그인 URL 발급·API 401·템플릿 카탈로그(17종) 확인.

  **파괴적 마이그레이션은 V62 하나였고 사전에 가드를 확인했다** — `pipelines`·`deployments` 에 행이 있으면 의도적으로 중단하는데 운영은 둘 다 0행이었다. 적용 후 두 테이블과 `users` 의 옛 컬럼 3개가 정상 제거됐다.

  **백업**: `dvely-prod-backup-20260918-101756.sql.gz`(286K) — 서버와 로컬 두 곳, md5 일치. 검증은 CREATE TABLE 31/31 · 완료 마커 · gzip 무결성으로 했다.

  **환경변수 정리도 함께**: `QEPLOY_AGENT_EXECUTOR_CORE_SIZE=2`/`MAX_SIZE=4`(RAM 3.8GB 에 프리뷰 컨테이너 1GB 상한이라 기본 4/8 은 과하다), `QEPLOY_AI_GLM_*`. 나머지 dev 전용 키들은 **운영에 옮길 필요가 없었다** — `gateway-base-url`·`frame-ancestors`·`require-access-cookie`·`bind-address` 모두 prod 프로파일 기본값이 이미 맞다(FE 가 게이트웨이와 같은 오리진이라 `'self'` 가 맞고, nginx 가 127.0.0.1:8080 으로 프록시한다).

  **설정 실수 두 건을 잡았다**: ① `ecosystem.config.js` 에 쉼표가 빠져 파싱이 깨져 있었다 — 앱은 예전 env 로 돌고 있어 증상이 없었고, 다음 배포에서야 "앱이 안 뜬다" 로 나타났을 것이다. ② `QEPLOY_AI_GLM_MODEL` 에 모델 두 개를 콤마로 넣었다(단일 값 자리다). 그 김에 **model 과 thinking-models 어긋남을 기동 때 경고**하도록 넣었다(PR #362) — 어긋나도 기동은 되고 요청 시 400 으로만 나타나, 설정한 사람과 증상 보는 사람이 다른 시점에 있는 형태였다.

  **남은 것**: 운영 프리뷰 e2e(로그인 필요), `qeploy-templates` 가 17종으로 늘어 있다(누가 추가했는지 확인 필요).

- 2026-09-15: **#332 4단계(egress 허용목록) 구현 — dev 에서 켜고 e2e 통과** (PR #360·#361). 이슈 전체(1~4단계)가 동작한다.

  **본문 제안대로 가면 격리가 깨진다는 것을 실측으로 가려냈다.** 공유 네트워크 하나로는 두 성질이 동시에 서지 않는다 — `enable_icc=false` 를 켜면 프리뷰가 **프록시에도 닿지 못하고**(허용 호스트 000), 끄면 프리뷰끼리 서로 닿는다. 코딩 에이전트는 포트도 안 열고 컨테이너 간 격리도 필요 없어 이 제약에 걸리지 않았다.

  **그래서 프리뷰마다 전용 `--internal` 네트워크를 주고 프록시 하나가 그 전부와 bridge 에 붙는다.** 격리는 `icc=false` 보다 강하다(다른 네트워크라 라우팅 경로 자체가 없다 — TCP·ICMP 모두 차단 확인). 게이트웨이는 무영향인데, **#358 에서 프록시 타깃을 컨테이너 IP 로 옮겨둔 것이 여기서 값을 했다**(`--internal` 에서는 포트 발행이 아예 동작하지 않는다).

  **씨딩이 `wget` → `curl` 로 바뀌었다(플래그와 무관하게 적용).** busybox wget 은 프록시 CONNECT 터널링을 하지 않아, 켜면 요청을 보내지도 않고 소켓을 닫는다 — tinyproxy 로그에 "Client closed socket before read" 로만 남는 형태다. 모드에 따라 도구를 가르지 않았다.

  **dev e2e(egress 켠 채)**: 세션 네트워크 `internal=true` 생성 · 프록시 기동(허용 11개) · 프리뷰가 전용 네트워크에만 · **허용 200 / 비허용 000** · 에러 없음 · 게이트웨이가 수정 결과 서빙.

  **켜 보고 버그 하나를 잡았다**(PR #361): apk 설치와 mkdir 을 detach 로 보내 놓고 바로 설정을 복사해 `Could not find the file /etc/tinyproxy` 로 죽었다. 준비 단계는 기다리고, 기동 뒤 실제 수신까지 확인한 뒤 진행한다 — 기다리지 않으면 첫 프리뷰의 apk·npm 이 "설치가 안 된다" 로만 실패해 프록시가 원인이라는 것이 드러나지 않는다.

  **dev 는 지금 egress 가 켜져 있다**(`QEPLOY_PREVIEW_EGRESS_ENABLED=true`). 기본값은 꺼짐이다.

  **남은 것**: ① **Node·JAVA_FULLSTACK 프로젝트를 켠 채로 태워 허용목록 확인** — 정적(vanilla 템플릿)만 검증했다. 빠진 호스트는 "설치가 안 된다" 로만 드러난다. ② 앱 경로의 네트워크 회수(`removeContainer` 의 훅)는 아직 실측 못 했다 — 검증 때 `docker rm -f` 로 앱을 우회했다. ③ 컨테이너가 밖에서 사라진 뒤 남는 고아 네트워크를 거두는 주기 작업이 없다.

- 2026-09-15: **#332 1~3단계 완료 + #358(게이트웨이 타깃 전환) 완료 — dev 라이브** (PR #356·#357·#359).

  **#332**: 프리뷰 컨테이너에서 사용자 저장소의 코드(npm postinstall·빌드 스크립트)가 root 로 돌던 것을 `node`(uid 1000)로 내렸다. 인계 문서의 확정 제약(`cap-drop ALL` 이 `DAC_OVERRIDE` 를 떼므로 **워크스페이스 주인은 하나여야 한다**)이 실측과 정확히 맞았다. 방향이 요점이었다 — 워크스페이스를 만지는 40곳을 하나씩 바꾸면 빠뜨린 곳만 조용히 깨지므로, **기본을 뒤집고 root 가 필요한 짧은 목록만 표시**했다(`installPackages` 로 apk 를 한곳에 모음 · nginx 기동/종료 · 생성 직후 mkdir+chown). 착수 전에 `docker exec -u root` 가 `no-new-privileges` 와 무관하게 동작하는지 먼저 쟀고, 그게 성립해서 이 방향이 가능했다. 인계 문서에 없던 root 필요 지점 2곳(**템플릿 씨딩·diff 기준 커밋**이 apk 를 명령 체인 안에 품고 있었다)을 찾아 채웠다. JAVA_FULLSTACK("가장 덜 검증된 곳")은 gradle 빌드(exit=0, 캐시가 `/home/node/.gradle`)→8080 기동→nginx 라우팅까지 실제 컨테이너로 전 구간 태웠다.

  **#358**: egress(4단계)가 구조에 막혀 있었다 — **`--internal` 네트워크에서는 포트 발행이 아예 동작하지 않는다**(실측: 매핑 미생성). 반면 호스트는 컨테이너 IP 로 닿는다(200). 그래서 게이트웨이 타깃을 `127.0.0.1:{host_port}` → `{container_ip}:3000` 으로 옮기고 **발행 자체를 없앴다.** 루프백 발행(BI-081/G1)보다 강한 상태다 — 묶을 포트 자체가 없다. 부수 효과로 **#278(host_port 재할당)이 다루던 원인이 사라졌다**(포트 고정 3000). 배포 순간의 세션은 지연 해석으로 끊기지 않는다. dev e2e: `container_ip=172.18.0.2 / host_port=NULL / 포트매핑=map[] / User=node`, 게이트웨이 경유 **200** 에 템플릿 수정 결과가 그대로 서빙됐다.

  **egress 와 한 PR 로 묶지 않은 이유**: 게이트웨이는 #274(회수/self-heal)·#278·#281(레디니스 게이트)이 쌓인 자리라, 함께 바꾸면 무엇이 깨졌는지 가려낼 수 없다.

  **남은 것**: #332 4단계 — `--internal` 전환 + 프록시 허용목록. 허용목록은 코딩 에이전트(`CodingAgentProperties.Egress`)보다 넓어야 한다(npm·yarn·pnpm·bun 레지스트리, GitHub clone·codeload·raw, Maven Central, Gradle 배포처, 템플릿 씨앗의 `dvely.github.io`). 이슈 본문이 경고한 대로 **빌드를 깨뜨릴 위험이 실질적이라 실제 프로젝트로 태워 보며** 해야 한다.
