...
- **컨트롤러 수**: 16개 · **공개 엔드포인트 수**: 93개 (Swagger 그룹 11개 + 프로젝트 그룹에 포함된 Approval/Change 하위 리소스)
...
- `status`가 `WAITING_APPROVAL`이면 프로젝트의 Chat 승인 정책(`GET /projects/{id}/settings/chat`)에 따라 사용자 승인이 먼저 필요합니다. **프로젝트 생성 시 5개 정책 모두 기본값 `true`**이므로, 별도로 끄지 않으면 대부분의 최초 작업은 승인 대기를 거칩니다.
...
CODE 단계는 GitHub 저장소 연결과 무관하게 동작합니다(Docker 프리뷰 컨테이너 기반) — 저장소 연결은 실제 배포(§5.4)를 위해서만 필요합니다.
...
```
① POST /api/v1/conversations/101/messages
{ "content": "버튼 색을 파란색으로 바꿔줘" }

→ 201 { "data": { "messageId": 3010, "conversationId": 101, "role": "user", ..., "taskId": "e5f6a1b2c3d4" } }
...
```
③ (선택) previewUrl로 결과 확인 + GET /api/v1/changes/{changeId}/diff 로 변경 diff 확인
   (changeId는 GET /api/v1/projects/12/changes 목록에서 taskId=e5f6a1b2c3d4로 매칭)
```

```
④-A 승인: POST /api/v1/approvals/52/approve
→ { "approvalId": 52, "type": "RESULT", "status": "APPROVED", "decidedAt": "..." }
   (내부적으로 preview→main PR 생성+merge, 해당 Change.status=MERGED,
    잔여 step이 있으면 task는 QUEUED로 재개, 없으면 바로 DONE)

④-B 거절: POST /api/v1/approvals/52/reject
→ { "approvalId": 52, "type": "RESULT", "status": "REJECTED", "decidedAt": "..." }
   (main에는 반영되지 않음 — 변경은 preview 브랜치에 그대로 남고, 해당 Change.status=REJECTED, task=CANCELLED)
```

```
⑤ (승인 시) GET /api/v1/agent/tasks/e5f6a1b2c3d4  (폴링 계속)
→ 잔여 step 없으면 바로 DONE. DEPLOY step이 남아 있으면 QUEUED → RUNNING → DONE
   (재개된 DEPLOY step은 merge를 다시 하지 않고, 이미 반영된 main을 그대로 공개만 합니다)

⑥ GET /api/v1/changes/{changeId}
→ { "status": "MERGED", "approvalId": 52, "prNumber": 12, "mergeCommitSha": "abcd123...", "mergedAt": "..." }
```
...
### 5.5 Environment secret CRUD (마스킹)
...
### 5.8 Cloud Ops (자연어 chat 경유 — INFRA_OPERATE)
...
```
리소스 스펙 변경(RESOURCE_SCALING)·오토스케일링·리소스 정리는 아직 실행이 지원되지 않아, 요청 시 승인 없이 즉시 "아직 지원하지 않음 + 대체 경로 안내"(예: §5.6의 인프라 설정 API로 유도)가 반환됩니다. 실 클라우드 서버 프로비저닝(BI-130)은 아직 구현되지 않았으므로, 모든 응답에 이 사실이 명시됩니다.
