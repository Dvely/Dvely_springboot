...
| Flyway | **V55** 사용. develop 이 V43·V44·V45·V47 을 연속 선점했다(아래 참고) | 조율 필요 |
...
> **번호 조율 기록 (네 번 밀렸다):** 착수 시점 최신이 V42 라 V43 을 예약했는데, develop 이 V43 → V44 → V45 → V47 을 연달아 가져가 결국 V55 가 됐다. 이 단위를 진행하는 동안 develop 은 마이그레이션을 12개 추가했다 — 예약이 왜 무의미한지 보여주는 수치다.
>
...
`V55__add_ai_provider_credentials.sql` (구현본):
```sql
...
1. **PR-1** 크리덴셜 도메인(`aiaccount/**`) + `CodingAgentPort` 정의 + **V55**. (신규 파일 위주)
