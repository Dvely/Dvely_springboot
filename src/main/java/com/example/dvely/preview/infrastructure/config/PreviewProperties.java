package com.example.dvely.preview.infrastructure.config;

import java.time.Duration;
import lombok.Getter;
import lombok.Setter;
import org.springframework.boot.context.properties.ConfigurationProperties;

@Getter
@Setter
@ConfigurationProperties(prefix = "qeploy.preview")
public class PreviewProperties {

    private String gatewayBaseUrl = "http://localhost:8080";
    private Duration ttl = Duration.ofMinutes(30);

    /**
     * 사람의 승인을 기다리는 태스크의 세션에 주는 만료 유예. 승인 종류를 가리지 않는다.
     *
     * <p>기본 TTL 은 사람이 화면을 보고 있다는 전제로 잡은 값이고, 접근할 때마다
     * {@code touch} 로 갱신된다. 승인 대기가 그 전제가 깨지는 자리다 — 사용자는
     * 프리뷰를 열어보지 않고 채팅의 승인 카드만 보고 결정할 수 있고, 그동안 세션은 아무에게도
     * 접근되지 않은 채 30분에 회수된다. 그런데 이 승인의 대상 작업물은 아직 GitHub 에 없고
     * 컨테이너 안에만 있으므로, 회수되는 순간 승인해도 올릴 것이 없다.</p>
     *
     * <p>한때 저장소 연결 승인에만 걸려 있었다. 그때 이 주석은 그것이 "유일한 자리"라고
     * 적고 있었지만 사실이 아니었고, 결과 반영 승인에서 같은 사고가 그대로 재현됐다
     * (#376: 2026-09-22 dev 에서 결과 승인 대기 진입 정확히 30분 뒤 세션이 EXPIRED).
     * 사람을 기다리는 상태를 새로 만들 때는 그동안 무엇이 만료되는지 같이 봐야 한다.</p>
     *
     * <p>무한정 붙들지 않고 유한한 값인 것도 의도다. 답이 오지 않는 승인이 컨테이너를 영원히
     * 붙들면 디스크가 찬다. 이 시간이 지나 회수되면 {@code LostPreviewApprovalSweeper} 가 남은
     * 승인을 닫으며 이 값을 사유에 적는다 — 승인할 대상이 사라졌는데 카드만 떠 있으면 사용자는
     * 누르면 실패하는 버튼을 보게 된다(#380).</p>
     *
     * <p>2026-09-22 까지 이 자리에는 "{@code ApprovalCommandService} 가 사유를 말하며 닫는다"고
     * 적혀 있었지만 <b>그런 코드는 없었다</b>. 의도만 적고 구현하지 않은 서술이라, 읽는 사람이
     * 후속 작업이 끝났다고 믿게 만들었다. 같은 문단의 다른 문장도 같은 이유로 틀려 있었다(#376).</p>
     */
    private Duration approvalHold = Duration.ofHours(6);

    /**
     * 게이트웨이가 소유권 쿠키를 요구할지 (Issue #77 G2).
     *
     * <p>기본값이 {@code true}인 것은 의도적이다 — 끄면 "URL을 아는 자 = 접근 허가" 모델로 되돌아간다.
     * FE가 접근 발급 호출을 아직 배포하지 못한 환경에서만 임시로 끄고, 끈 동안에는 그 갭이 그대로
     * 열려 있다는 것을 알고 있어야 한다.</p>
     */
    private boolean requireAccessCookie = true;
}
