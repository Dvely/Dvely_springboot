...
public class WebImageBuildService {
    ...
    public Path buildWebImageTar(Long ownerUserId, Long projectId, String frontendRepo,
        ...
        Path contextTar = prepareWebContextTar(ownerUserId, projectId, frontendRepo, frontendDir, apiPathPrefix);
        try {
            Path imageTar = imageBuilder.buildTar(contextTar, tag);
            log.info("웹 이미지 빌드 완료(S3 전달): projectId={} tag={}", projectId, tag);
            return imageTar;
        } finally {
    ...
    public void buildAndPushWebImage(Long ownerUserId, Long projectId, String frontendRepo,
        ...
        try {
            ...
            log.info("웹 이미지 빌드·푸시 완료(ECR 전달): projectId={} ref={}", projectId, imageRef);
        } finally {
            imageBuilder.deleteQuietly(contextTar);
        }
    }
     ...
     * frontendRepo 를, 모노는 백엔드 레포의 frontendDir 를 프론트 루트로 삼는다.
     */
    private Path prepareWebContextTar(Long ownerUserId, Long projectId, String frontendRepo,
        ...
        try {
            ...
            ensureWebAssets(containerId, frontendRoot, apiPathPrefix);
     ...
     */
    private void ensureWebAssets(String containerId, String frontendRoot, String apiPathPrefix) {
        ...
        List<String> prefixes = WebAssetsFactory.parsePrefixes(apiPathPrefix);
