...
class BackendDeployAgentServiceTest {
    ...
    void provisionsDatabaseAndServerWhenDbRequestedAndNoneExists() {
        ...
        when(serverCommandService.submit(USER, PROJECT, null, ServerDeployMode.NATIVE, null))
        ...
        verify(serverCommandService).submit(USER, PROJECT, null, ServerDeployMode.NATIVE, null);
    ...
    void serverOnlyWhenNoDbEngine() {
        when(serverCommandService.submit(USER, PROJECT, "t3.small", ServerDeployMode.NATIVE, null))
                .thenReturn(new ServerProvisionSubmitResult(true, 5L, List.of(2L)));
        ...
        verify(databaseCommandService, never()).provision(any(), any(), any(), any());
        verify(serverCommandService).submit(USER, PROJECT, "t3.small", ServerDeployMode.NATIVE, null);
    }
    ...
    void skipsWhenProjectMissing() {
        ...
        verify(serverCommandService, never()).submit(any(), any(), any(), any(), any());
    }
